...
impl Terminal {
    ...
    pub fn start(&mut self) -> Result<()> {
        ...
        let cfg = self.config.borrow::<TerminalConfig>()
            .map_err(|_| crate::error::OxError::TerminalConfig { 
...
pub fn rgb_to_xterm256(r: u8, g: u8, b: u8) -> u8 {
    ...
    for (&index, &(xr, xg, xb)) in lookup.iter() {
...
/// Parse XTERM lookup data at compile time to avoid runtime panics
fn parse_xterm_lookup() -> HashMap<u8, (u8, u8, u8)> {
    let mut result = HashMap::default();
    for line in XTERMLOOKUP.split('|') {
        let mut parts = line.split(':');
        let Some(id_str) = parts.next() else {
            eprintln!("WARNING: XTERM lookup format error: missing ID in line '{line}'");
            continue;
        };
        let Some(rgb_str) = parts.next() else {
            eprintln!("WARNING: XTERM lookup format error: missing RGB in line '{line}'");
            continue;
        };
        
        let Ok(id) = id_str.parse::<u8>() else {
            eprintln!("WARNING: XTERM lookup: invalid ID format '{id_str}'");
            continue;
        };
        ...
        let r = rgb_parts.next()
            ...
            .unwrap_or_else(|| {
                eprintln!("WARNING: XTERM lookup: invalid R value in '{rgb_str}'");
                0
            });
        let g = rgb_parts.next()
            .and_then(|s| s.parse::<u8>().ok())
            .unwrap_or_else(|| {
                eprintln!("WARNING: XTERM lookup: invalid G value in '{rgb_str}'");
                0
            });
        let b = rgb_parts.next()
            .and_then(|s| s.parse::<u8>().ok())
            .unwrap_or_else(|| {
                eprintln!("WARNING: XTERM lookup: invalid B value in '{rgb_str}'");
                0
            });
...
/// Lazy-loaded XTERM color lookup table
static XTERM_LOOKUP: Lazy<HashMap<u8, (u8, u8, u8)>> = Lazy::new(parse_xterm_lookup);

/// Get the XTERM lookup table
pub fn get_xterm_lookup() -> &'static HashMap<u8, (u8, u8, u8)> {
    &XTERM_LOOKUP
}
...
pub fn remove_ansi_codes(input: &str) -> String {
    use regex_cache::ansi::*;
    // Replace all matches with an empty string
    let result = GLOBAL_ANSI.replace_all(input, "").to_string();
...
pub fn strip_escape_codes(input: &str) -> String {
    ...
    let result = GLOBAL_ANSI
        .replace_all(input, |caps: &regex::Captures| {
            let code = caps.get(0).map(|m| m.as_str()).unwrap_or("");
            if DISPLAY_ONLY.is_match(code) {
...
pub fn replace_reset(input: &str, custom_bg: &str, custom_fg: &str) -> String {
    ...
    let input = TOTAL_RESET.replace_all(input, full_reset).to_string();
    let input = RESET_BG.replace_all(&input, custom_bg).to_string();
    let input = RESET_FG.replace_all(&input, custom_fg).to_string();
    input
}
